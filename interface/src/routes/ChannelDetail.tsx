			...
			<div className="min-w-0 flex-1">
				<div className="rounded-md bg-violet-500/10 px-3 py-2">
					<div className="flex items-center gap-2">
						...
						<span className="truncate text-sm text-ink-dull">
							{item.description}
						</span>
						<CancelButton
							onClick={() => {
...
function TimelineEntry({
	...
	switch (item.type) {
		...
		case "branch_run": {
			...
			if (live)
				return (
					<LiveBranchRunItem item={item} live={live} channelId={channelId} />
				);
		...
		case "worker_run": {
			...
			if (live)
				return (
					<LiveWorkerRunItem item={item} live={live} channelId={channelId} />
				);

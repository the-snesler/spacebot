...
| `mcp_*` | One tool per connected MCP server tool, fetched at worker start |
...
Workers can also be backed by external coding-agent integrations instead of the built-in Rig worker. Spacebot currently supports OpenCode workers and ACP workers.

See [Code Workers](/docs/code-workers) for configuration, tradeoffs, and lifecycle details.

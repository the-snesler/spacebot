...
OpenCode workers and ACP workers apply the same output scrubbing, and external worker output is also scanned for secret patterns for observability:
...
ACP profile `command` and `env` values also go through Spacebot's config resolution, so `env:VAR_NAME` and `secret:NAME` references are expanded before the subprocess launches.
